...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        // 替换 replaceCharactersInRange:range:
        NSString *tmpReplaceRangeOfStr = @"replaceCharactersInRange:withString:";
        NSString *tmpSafeReplaceRangeOfStr = @"safe_replaceCharactersInRange:withString:";
        
        [NSObject exchangeInstanceMethodWithSelfClass:NSClassFromString(@"__NSCFString")
                                     originalSelector:NSSelectorFromString(tmpReplaceRangeOfStr)                                     swizzledSelector:NSSelectorFromString(tmpSafeReplaceRangeOfStr)];
        
        // 替换 stringByReplacingCharactersInRange:range:
        tmpReplaceRangeOfStr = @"stringByReplacingCharactersInRange:withString:";
        tmpSafeReplaceRangeOfStr = @"safeMutable_stringByReplacingCharactersInRange:withString:";
        
        [NSObject exchangeInstanceMethodWithSelfClass:NSClassFromString(@"__NSCFString")
                                     originalSelector:NSSelectorFromString(tmpReplaceRangeOfStr)                                     swizzledSelector:NSSelectorFromString(tmpSafeReplaceRangeOfStr)];
        
        // 替换 insertString:atIndex:
        NSString *tmpInsertIndexOfStr = @"insertString:atIndex:";
        NSString *tmpSafeInsertIndexOfStr = @"safe_insertString:atIndex:";
        
        [NSObject exchangeInstanceMethodWithSelfClass:NSClassFromString(@"__NSCFString")
                                     originalSelector:NSSelectorFromString(tmpInsertIndexOfStr)                                     swizzledSelector:NSSelectorFromString(tmpSafeInsertIndexOfStr)];
        
        // 替换 deleteCharactersInRange:
        NSString *tmpDeleteRangeOfStr = @"deleteCharactersInRange:";
        NSString *tmpSafeDeleteRangeOfStr = @"safe_deleteCharactersInRange:";
        
        [NSObject exchangeInstanceMethodWithSelfClass:NSClassFromString(@"__NSCFString")
                                     originalSelector:NSSelectorFromString(tmpDeleteRangeOfStr)                                     swizzledSelector:NSSelectorFromString(tmpSafeDeleteRangeOfStr)];
        
        // 替换 stringByAppendingString:
        NSString *tmpAppendingOfStr = @"stringByAppendingString:";
        NSString *tmpSafeAppendingOfStr = @"safeMutable_stringByAppendingString:";
        
        [NSObject exchangeInstanceMethodWithSelfClass:NSClassFromString(@"__NSCFString")
                                     originalSelector:NSSelectorFromString(tmpAppendingOfStr)                                     swizzledSelector:NSSelectorFromString(tmpSafeAppendingOfStr)];
    });
...
- (NSString *)safeMutable_stringByReplacingCharactersInRange:(NSRange)range withString:(NSString *)replacement
{
    if (!replacement) {
        return self.copy;
    }
    ...
    return [self safeMutable_stringByReplacingCharactersInRange:range withString:replacement];
}
...
- (NSString *)safeMutable_stringByAppendingString:(NSString *)aString
{
    if (!aString) {
        return self.copy;
    }
    
    return [self safeMutable_stringByAppendingString:aString];
}
